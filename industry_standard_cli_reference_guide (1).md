...
### Syntax 
```
max-metric router-lsa { [ administrative ] | { [ on-startup <interdistance> ] } | { [ all [ <maxmetricvalue> ] ] } | { [ include-stub [ <maxmetricvalue> ] ] } }
no max-metric router-lsa { [ administrative ] | [ on-startup ] | [ all ] | [ include-stub ] }
```
### Parameters 
| Name | Description | Type |
|:---:|:-----:|:-----:|
...
### Syntax 
```
radius-server host <host> [ auth-port <vauth_port> ] [ auth-type <vauth_type> ] [ key <vkey> ] [ priority <vpriority> ] [ retransmit <vretransmit> ] [ source-interface { Ethernet | Loopback | Management | PortChannel | Vlan | SubInterface } ] [ timeout <vtimeout> ] [ vrf { mgmt | <vrf-name> } ]
...
```
show interface { { counters { [ rate ] | [ <phy-if-name> ] | [ <po-name> ] ] } } | { Eth [ <iface_num> ] } | { Ethernet [ <iface_num> ] } | <iface_range_num> | { PortChannel [ <po-id> ] } | { Management [ <mgmt-if-id> ] } | <vlan_range_num> | { Loopback <lo-id> } | status | { transceiver [ <phy-if-name> ] [ summary ] { [ diagnostics { capability | status } ] } } }
